...
5. **startup / recovery 前台摘要必须优先消费结构化 truth**；例如 startup summary、project supervision、runtime pickup。不得把 `startup config ...`、`cfg(...)` 这类内部摘要原样当作默认用户文案
...
- **不得**把 framework-owned heartbeat / tick / supervisor / daemon / routing 这类 control-plane 观察事件继续落到正常 session 的 `events/stream.jsonl` 或 archive segment
