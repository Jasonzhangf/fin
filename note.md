...
- Follow-up device evidence showed send can still wedge after reconnect: native log had `native_input_send`/`device_e2e_sent`, then connection replacement and many `native_ws.stale_message generation=1`; old turn result frames were dropped before UI consumed `turn.completed/turn.rendered`. Android owner fix should forward stale turn lifecycle/result frames only, keep health/session noise stale-dropped, hide native composer for settings/sessions, and make update UI semantic instead of showing bridge JSON.
- 2026-06-09 12:51 server follow-up: Android receive/render gap is not send-only anymore. True provider run showed `input.accepted -> turn.started -> provider_wait -> turn.completed -> turn.rendered`, but `/ws` still emitted no `turn.item.*` because debug-server rendered `tool_execution_records: []`. Owning fix is debug-server mobile projection from current operation `session_recent_tool_records_path`, with WS contract test requiring `turn.item.completed/failed` before final render.

[2026-06-09] Android foreground/render gap: user screenshot shows normal active inference can render, but background -> foreground loses rendered conversation. Current code only CONFIG.load() on onResume; it does not force mobile.subscribe/session.history rehydrate or reconnect, so WebView can show stale/blank projection after lifecycle restore. Separately, server WS emits turn.progress heartbeats during provider wait but only projects turn.item.* after handler completion from recent_tool_records, so per-term live tool/provider rendering is late. Fix plan: Android foregroundResume() = CONFIG.load + loadProfiles + force reconnect/resubscribe/rebind; progressTermFromPhase/upsertProgressItem creates running provider/tool term from progress frames; debug-server emits provider.call turn.item.started at WS turn start and completes same item after handler result.

[2026-06-09] Verified Android realtime/foreground closeout: debug-server 0.1.10027 projects mobile `session.history` from runtime `assistant_visible_output` into `assistant_response`, emits `turn.item.started provider.call running` immediately after `turn.progress`, then completes the same provider item from `recent_tool_records`. Android app now forces foreground reconnect/resubscribe/history render, consumes `assistant_visible_output` for restored turns, displays running/completed provider/tool terms, surfaces failed turns, and consumes `finAutoSend` only once. Device 100.127.23.27:1234 evidence: live marker `PING-ANDROID-LIVE-20260609-135913` rendered running Provider Call before final, `last_run.answer` matched marker with provider=minimax model=MiniMax-M3, and foreground resume logged `app.foreground_resume -> handshake.ok -> ui_turn_rendered` while screenshots showed restored assistant bubbles without `未返回内容`.
