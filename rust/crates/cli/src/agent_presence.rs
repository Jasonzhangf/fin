...
fn presence_path(runtime_home: &Path, agent_id: &str) -> std::path::PathBuf {
    runtime_home.join(format!("runtime/agents/state/{agent_id}.json"))
}

fn write_presence(runtime_home: &Path, record: &AgentPresenceRecord) -> Result<(), CliError> {
...
fn read_presence(path: &Path) -> Result<Option<AgentPresenceRecord>, CliError> {
    match fs::read_to_string(path) {
