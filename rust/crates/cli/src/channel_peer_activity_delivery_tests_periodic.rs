use super::*;
use crate::channel_peer_progress_policy::save_qqbot_progress_policy;
...
fn periodic_delivery_emits_once_for_changed_snapshot() {
    ...
    assert!(first.text.contains("📡 状态卡 · system"));
    ...
    mark_delivered(
        ...
        &first.compact_text,
        &first.reason,
        first.disclosed_task_list_signature.as_deref(),
    )
...
fn periodic_delivery_skips_when_rendered_text_is_unchanged_even_if_signature_differs() {
    ...
    let mut policy = crate::channel_peer_progress_policy::QqbotProgressPolicy::default();
    policy.heartbeat_interval_secs = 0;
    save_qqbot_progress_policy(&runtime_home, policy).expect("save policy");
    ...
    write_json(
        ...
        &serde_json::json!({
            ...
            "last_compact_text": current.compact_text,
...
fn periodic_delivery_stays_silent_when_text_is_unchanged_even_if_interval_elapsed() {
    ...
    let mut policy = crate::channel_peer_progress_policy::QqbotProgressPolicy::default();
    policy.heartbeat_interval_secs = 1;
    save_qqbot_progress_policy(&runtime_home, policy).expect("save policy");
    ...
    write_json(
        ...
        &serde_json::json!({
            ...
            "last_compact_text": first.compact_text,
...
fn periodic_delivery_does_not_repeat_stale_waiting_failure_snapshot() {
    let runtime_home = temp_runtime_home();
    let session_rel = "sessions/2026/05/session-stale-heartbeat";
    write_json(
        &runtime_home.join("runtime/current/last_run.json"),
        &serde_json::json!({
...
fn periodic_delivery_discloses_task_list_detail_once_until_task_set_changes() {
    let runtime_home = temp_runtime_home();
    let session_rel = "sessions/2026/05/session-task-list-memory";
    write_json(
        &runtime_home.join("runtime/peers/registry.json"),
        &serde_json::json!({"peers":[]}),
    );
    ...
    mark_delivered(
        ...
        &second.compact_text,
        &second.reason,
        second.disclosed_task_list_signature.as_deref(),
    )
    .expect("mark second");
...
fn periodic_delivery_stays_bound_to_frontstage_session_when_worker_session_becomes_last_run() {
    let runtime_home = temp_runtime_home();
    let frontstage_rel = "sessions/2026/05/session-frontstage";
    let worker_rel = "sessions/2026/05/session-worker";
    write_json(
        &runtime_home.join("runtime/current/last_run.json"),
        &serde_json::json!({
