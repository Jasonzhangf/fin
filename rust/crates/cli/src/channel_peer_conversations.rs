use crate::{
    ...
    session_ledger_read::{read_latest_snapshot_message, session_exists_by_ledger},
...
pub(crate) const QQBOT_CHANNEL_ID: &str = "qqbot";
...
pub(crate) fn conversation_id_for(target: &str) -> String {
...
fn session_exists(runtime_home: &Path, session_id: &str) -> bool {
    session_exists_by_ledger(runtime_home, session_id)
}
...
fn latest_deliverable_message(
    ...
    if let Some(session_id) = session_id {
        if let Some(message) = read_latest_snapshot_message(runtime_home, session_id)? {
            return Ok(Some((message.message_id, message.created_at)));
        }
    }
