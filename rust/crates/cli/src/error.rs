...
pub enum CliError {
    #[error(
        "usage: fin <config-check|home-init|control-boundary-demo|mainline-demo|runtime-demo|debug-projection|transcript-demo|provider-live-smoke|web-debug|build-dev|install-dev|promote|rollback> <user.toml> [input|port|build-version|transcript.json]"
    )]
    Usage,
