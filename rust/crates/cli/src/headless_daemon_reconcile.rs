...
pub(crate) fn reconcile_headless_daemon_state(
    ...
    let execution_lease = read_json_if_exists::<ActiveExecutionLease>(
    ...
    let active_execution_lease = execution_lease
        .as_ref()
        .filter(|value| value.status == "active" && process_alive(value.pid));
    if alive && !stale_lease {
        return Ok(None);
    }

    if let Some(execution_lease) = active_execution_lease {
        ...
        let summary = format!(
            "daemon executing active closure pid={} session={} stale_daemon_lease={} startup={}",
            execution_lease.pid,
            execution_lease.session_id.as_deref().unwrap_or("-"),
            stale_lease,
            startup.status_summary(),
        );
        let active_state = DaemonStateRecord {
            daemon_id: daemon_id.clone(),
            created_at: now.clone(),
            updated_at: now.clone(),
            refs: EntityRefs {
                session_id: execution_lease.session_id.clone(),
                ..EntityRefs::default()
            },
            service_kind: "headless_daemon".into(),
            lifecycle_state: "active".into(),
            supervision_state: "running_sessions".into(),
            mode: "detached".into(),
            pid: Some(execution_lease.pid),
            last_heartbeat_id: None,
            last_cycle_id: None,
            health_state: Some("healthy".into()),
            recovery_needed: false,
            recovery_action_kind: Some("observe_only".into()),
            active_binding: execution_lease
                .session_id
                .as_ref()
                .map(|session_id| format!("session={session_id}")),
            status_summary: summary.clone(),
        };
        persist_daemon_state(&paths, &active_state)?;
        let recovery = DaemonRecoveryActionRecord {
            action_id: format!(
                "{}-recovery-{}",
                daemon_id,
                crate::demo::sanitize_id_fragment(&now)
            ),
            created_at: now,
            refs: EntityRefs {
                session_id: execution_lease.session_id.clone(),
        ...
        fs::write(&paths.pid_path, execution_lease.pid.to_string()).map_err(|source| {
            CliError::WriteFile {
        ...
        if stale_lease {
            let _ = fs::remove_file(&paths.lease_path);
        }
        return Ok(Some(HeadlessDaemonReconcileOutcome {
            state: active_state,
