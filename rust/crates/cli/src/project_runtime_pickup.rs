...
fn derive_pickup_state(
    ...
    match state.map(|value| value.status.as_str()) {
        ...
        _ if handoff_idle_after_resume(handoff, state) => (
            "handoff_idle".into(),
            "await_new_project_input".into(),
            format!(
                "project runtime already drained handoff for task {}; waiting for new project input",
                handoff.task_id.as_deref().unwrap_or("-")
            ),
        ),
        _ => (
...
fn handoff_idle_after_resume(
    handoff: &ProjectExecutionHandoffRecord,
    state: Option<&fin_contracts::ExecutionStateRecord>,
) -> bool {
    if handoff.handoff_state == "noop" {
        return true;
    }
    handoff.handoff_state == "prepared"
        && state
            .map(|value| value.updated_at.as_str() > handoff.updated_at.as_str())
            .unwrap_or(false)
}
