        ...
        .and_then(|step_id| step_id.rsplit_once('_').map(|(_, suffix)| suffix))
    {
        ...
        _ => {
            let summary = record
                ...
                .and_then(serde_json::Value::as_str)
                .unwrap_or_default();
            if summary.starts_with("assembled context") {
                "context_build"
            } else if summary.contains("parsed provider output") {
                "model_parse"
            } else if summary.contains("control feedback") {
                "control_feedback"
            } else if summary.contains("executed") && summary.contains("tool calls") {
                "tool_dispatch"
            } else if summary.starts_with("closure finalized") {
                "finalize"
            } else if summary.contains("provider ") {
                "provider_request"
            } else {
                "other"
            }
