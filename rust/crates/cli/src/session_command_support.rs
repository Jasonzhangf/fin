...
#[derive(Debug, Clone)]
pub(super) struct RebindResult {
    pub(super) session_messages_path: Option<String>,
    pub(super) recent_contexts_path: Option<String>,
    pub(super) recent_digests_path: Option<String>,
}

pub(super) fn rebind_last_run(
    runtime_home: &Path,
    session_id: &str,
    task_id: &str,
    year: i32,
...
pub(super) fn infer_session_task_id(session_dir: &Path) -> Option<String> {
...
pub(super) fn ensure_json_file(path: &Path, default: &[u8]) -> Result<(), CliError> {
    if path.exists() {
        return Ok(());
    }
...
pub(super) fn relative_to_runtime(runtime_home: &Path, path: &Path) -> Option<String> {
    path.strip_prefix(runtime_home)
        .ok()
        .map(|value| value.to_string_lossy().to_string())
}

pub(super) fn trim_head<T>(items: &mut Vec<T>, limit: usize) {
    if items.len() > limit {
...
pub(super) fn read_json_or_empty<T: for<'de> serde::Deserialize<'de>>(
    path: &Path,
