...
mod tests {
    ...
    fn parse_turn_index_reads_suffix_digits() {
        assert_eq!(parse_turn_index("op-session-0009"), Some(9));
        assert_eq!(parse_turn_index("op-session"), None);
    }
    ...
    fn next_turn_index_prefers_last_run_counter() {
        ...
        assert_eq!(
            next_turn_index(Some(&last_run), Some("op-session-0008"), &[]),
            10
        );
    }
    ...
    fn next_turn_index_falls_back_to_observed_history() {
        let observed = vec!["op-session-0001".into(), "op-session-0009".into()];
