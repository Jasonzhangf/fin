...
#[test]
fn anthropic_response_rejects_null_content_as_schema_error() {
    let request = PreparedRequest {
        provider_name: "minimax".into(),
        protocol: ProviderProtocol::AnthropicWire,
        endpoint: "https://api.minimaxi.com/anthropic/v1/messages".into(),
    ...
    let err = provider_wire::parse_anthropic_response(
        &request,
        200,
        r#"{"id":"msg-null","type":"message","content":null,"stop_reason":"end_turn"}"#,
    )
    .expect_err("null content must not become empty success");

    assert!(matches!(err, ProviderError::ParseResponse { .. }));
    assert!(err.to_string().contains("content must be an array"));
    assert!(err.to_string().contains("provider=minimax"));
    assert!(err.to_string().contains("model=MiniMax-M3"));
    assert!(err.to_string().contains("response_id=msg-null"));
}
