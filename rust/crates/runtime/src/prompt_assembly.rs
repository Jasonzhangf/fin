...
fn role_baseline_modules(role_id: &str) -> Vec<PromptModuleEntry> {
    match role_id {
        "system" => vec![
            ...
            prompt_module(
                "role.system.cross_cwd_dispatch",
                "System Role / Cross Cwd Dispatch",
                "if the request requires reading, changing, testing, or auditing a different project root / cwd than the current primary scope, treat it as project-management work: inspect peers and supervision, choose or wake the matching project agent, dispatch a bounded task, and wait for explicit result refs before reporting".into(),
                "role_profile",
                90,
            ),
        ],
        _ => vec![
            ...
            prompt_module(
                "role.project.worker_execution_modes",
                "Project Role / Execution Modes",
                "inside the same project role, adapt between execution, review, diagnosis, and handoff; use local workers/subagents for bounded slices but keep project truth, review, and delivery closure in the project agent".into(),
                "role_profile",
                90,
            ),
        ],
...
fn behavior_rules(role_id: &str, loaded_skills: &[LoadedSkill]) -> Vec<String> {
    let mut rules = vec![
        ...
        "preserve conversational continuity across turns; treat the current turn as part of an ongoing agent conversation unless framework truth shows a real topic shift".into(),
        "when you delegate, dispatch, wait, recover, or review, continue supervising that work item until explicit completion, failure, timeout, or user-visible recovery; do not silently stop after the first tool call".into(),
        "turn progress, tool progress, and peer/project-agent progress should be surfaced as new forward timeline updates, not by rewriting prior history".into(),
        "treat session delivery as a continuous timeline thread: the next assistant action must connect to the latest user-visible turn, live progress, and delegated progress instead of restarting as an isolated question-answer block".into(),
        "treat tool calls as steps inside one managed execution loop: inspect current truth, decide the next action, act with the right framework tool, verify the result, then continue or close explicitly".into(),
        "managed execution recipe: inspect framework truth, state the immediate intent, choose one tool/action, review the returned artifact/event, then explicitly continue, wait, recover, review, or close".into(),
        "before the first framework tool call in a turn, state the routing/execution intent in the assistant response so the user can understand why this tool step is happening and what result will be returned".into(),
        "after each framework tool result, decide the next action explicitly: continue, wait, recover, dispatch, review, or close; do not stop at raw tool output or assume the framework will narrate for you".into(),
    ...
    match role_id {
        "system" => rules.extend([
            ...
            "before routing, dispatch, reprioritization, or recovery, inspect framework-owned task board, agent presence, supervision, and peer state instead of inferring control state from chat text alone".into(),
            "before cross-project or cross-cwd routing, inspect peer.list, peer.describe, agent.presence.list, and project.supervision.list so the target project agent is chosen from framework truth instead of memory".into(),
            "if work requires a different project root / cwd than the current primary scope, route it to the matching project agent; do not perform substantial cross-cwd execution inside the system session beyond lightweight existence checks needed for routing".into(),
            ...
            "for managed routing, inspect project.task.list first, then use agent.assign or equivalent project harness dispatch to send a bounded task and wait for explicit result refs before reporting completion".into(),
            "preferred system routing tool chain when another project root is needed: inspect peer.list -> inspect peer.describe/project.supervision.list -> ensure or wake the target with daemon.ensure_peer if needed -> dispatch bounded work with agent.assign -> keep supervising until result refs arrive".into(),
            "after dispatching to a project agent or peer, keep the user-facing conversation alive with supervised progress, health checks, completion review, and recovery decisions instead of treating dispatch as terminal success".into(),
            "after delegated work starts, explicitly connect each new update to the current session thread: progress, wait state, failure, recovery, and final result should arrive as append-only continuation updates until the delegated run is closed".into(),
            "when you route to a project agent, narrate the managed loop in-order: why this project is chosen, what bounded task was dispatched, what status signal you are waiting for, and how you will review the returned result".into(),
        ...
        _ => rules.extend([
            ...
            "use project.task.list, project.task.claim, project.task.submit, and project.task.review as the primary project truth for execution, submission, and review state".into(),
            "when the work stays in the same project but benefits from parallel execution, dispatch a bounded slice with agent.assign or project-local subagent execution instead of expanding the main thread without limit".into(),
            "when local workers or subagents run, keep supervising their progress and report new timeline updates until review and delivery closure are explicit".into(),
            "project-agent progress must continue the existing delegated session thread with append-only updates; do not answer as if the delegated task were an unrelated fresh conversation".into(),
            "inside the project role, use tool calls as steps inside one managed execution loop: inspect project truth, choose one bounded next action, execute it, verify artifacts/tests/events, then either continue the same thread or close with evidence".into(),
            "managed project execution recipe: claim or inspect task truth first, pick one bounded next step, execute or delegate it, verify artifacts/tests/events, then explicitly submit/review/continue/wait instead of silently ending the run".into(),
