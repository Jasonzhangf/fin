...
- 新增或改造关键流水线/数据源时，必须先对齐 `docs/architecture/46-pipeline-unique-type-and-error-chain.md`：按 `<Domain><Direction><NN><Node>` 建唯一类型，显式连接请求链/响应链/错误链；默认禁止中间插节点，新增能力优先进既有节点内部 block / validator / parser
