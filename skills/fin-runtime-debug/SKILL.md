...
- 若 `provider-live-smoke` 报 `unsupported protocol for real execution: OpenAiCompatible`，第一反应不是补静态 mock，而是确认 `ProviderFacade::execute_prepared` 是否实现了 OpenAI 分支；pipeline unique type 重构很容易吞掉协议分支，**`execute_prepared` 协议分支覆盖必须等于 `ProviderRegistry` 已注册协议集合**，缺一就视为 silent drop。
- 若 `status >= 400` 路径直接 `return Err(HttpStatus)` 而不分类，违反硬护栏 #15（429/5xx/usage limit 必须 1s*2^(n-1) 重试 5 次）。任何时候改 facade 的 status 处理，**必须**在 `http_client::classify_http_status` 收口一次，覆盖 429/503/5xx + 配额/usage limit/余额不足 body 关键词，干净 4xx 不重试。
- 重试测试必须真打 `TcpListener` 验证 `attempt` 次数和 `backoff` 间隔（`Duration::from_secs(1 << (attempt-1))`）；**不要**用 `tokio::time::pause()` + `mock_request` 当作真实验证。
- Android 前后台恢复后若历史 turn 只剩用户气泡或显示“未返回内容”，先查 `session.history` 的 mobile contract：runtime `TurnRecord` 的 `assistant_visible_output` 必须在 debug-server mobile projector 中转成 `assistant_response`，并补齐 `tool_execution_records/error_records` 数组；不要只在 WebView 末端补样式。
